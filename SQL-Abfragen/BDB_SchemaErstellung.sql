...
CREATE TABLE bdb_Kassensystem.arbeitetIn(
    ...
    FOREIGN KEY (FilialID) REFERENCES bdb_Immobilienverwaltung.Filiale(FilialID)(FilialID),
    FOREIGN KEY (VerkäuferID) REFERENCES bdb_Kassensystem.Verkäufert(VerkäuferID)
...
CREATE TABLE bdb_Kassensystem.wirdVerkauft(
    ...
    ProduktID INT NOT NULL,
    Menge DECIMAL(5,2),
    PRIMARY KEY (FilialID, Zeitpunkt, ProduktID),  -- Zusammensetzter Primärschlüssel
    FOREIGN KEY (FilialID) REFERENCES bdb_Immobilienverwaltung.Filiale(FilialID)(FilialID),
    FOREIGN KEY (ProduktID) REFERENCES bdb_Kassensystem.Produkt(ProduktID),
